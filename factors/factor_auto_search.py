...
class LLMPromptFactorGenerator(FactorGenerator):
    ...
    def __init__(self,
        ...
        # LLM 将滚动标准化固化到公式 OpRollNorm 中（与 GeneticFactorGenerator 行为一致）
        ...
        self.llm_profile_name: Optional[str] = model_name  # 对应 .env 中 LLM_PROFILES 的 name
        self.cancel_event = None  # threading.Event, set externally to cancel LLM calls

    def _call_llm_prompt(self, prompt: str) -> str:
        """调用 LLM 获取回复文本，使用 utils.llm_utils（anthropic 格式）。"""
        from utils.llm_utils import call_llm
        return call_llm(
            system_prompt='你是期货量化研究助理。请只输出可执行的因子公式。',
            user_prompt=prompt,
    ...
    def generate_factor_df(self,
        ...
        llm = None  # not used directly anymore
        ...
        for round_idx in range(max_rounds):
            ...
            if self.cancel_event and self.cancel_event.is_set():
                log.info('LLM mining cancelled by user.')
                break
            ...
            try:
                content = self._call_llm_prompt(prompt)
        ...
        # 如果启用滚动标准化，将公式包裹 OpRollNorm（与 GeneticFactorGenerator 行为一致）
        if self.apply_formula_rolling_norm:
            valid_formula_list = [
                f'OpRollNorm({formula}, {self.rolling_norm_window}, {self.rolling_norm_min_periods}, {self.rolling_norm_eps}, {self.rolling_norm_clip})'
                for formula in valid_formula_list
            ]
